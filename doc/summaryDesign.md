...
+ 通知查询
    ...
    + 基本描述：
        ...
        企业端用户可以查询省、市端用户发来的通知。

    + 基本事件流：

        1.企业用户进入“查看通知”页面，选择想要查看的通知，点击“查看”按钮。

        2.后台服务器根据用户在前一步骤选择的通知，然后再数据库中调出该条通知相关信息（通知标题、通知内容和发布时间）。
...
+ 审核和上报数据
    ...
    + 基本事件流
      ...
      1. 用户点击”数据审核“导航，进入数据审核界面，可以看到全省各个市的概览上报情况，包括每个市已上报企业的数量和未上报企业的数量。
      2. 在数据审核界面，点击具体某一个市的已上报数量，进入已上报界面，可以查看该市已上报企业的概览信息列表。
      3. 在已上报界面，点击列表中某一个企业“查看”按钮，进入详细数据界面，可以查看该企业上报的具体数据。
      4. 在详细数据界面，若上报数据没有问题，点击“审核通过”按钮，状态变为“已审核”；再点击“上报
         按钮，状态变为“已上报”。
      5. 若有问题，点击“退回修改”按钮，并输入退回修改理由，提交后返回已上报界面，并将该企业数据从列表移除。
      6. 在数据审核界面，点击具体某一个市的未上报数量，进入未上报界面，可以查看该市未上报企业的基本信息，同时可以查看退回修改企业的信息，并下拉查看之前上报的数据。
    
+ 发布通知

    - 基本描述：

      省端用户可以新建发布通知给全体企业端用户。

    - 基本事件流：

      1.用户进入“通知页面”页面，选择下方的“新建”按钮。

      2.网页跳转到“新增通知”页面，用户开始编写通知标题和通知内容。

      3.用户编写完成后，点击确定按钮，后台程序根据点击的时间生成发布时间，将通知信息传送到后台服务器数据库中，并且网页跳转到“通知页面”页面。

+ 编辑通知

    - 基本描述：

      省端用户可以编辑已有的通知，改变通知标题和内容。

    - 基本事件流：

      1.用户进入“通知页面”页面，选择列表中的想要编辑的通知，点击通知左侧的编辑按钮。

      2.网页跳转到“编辑通知”页面，用户开始编辑已有的通知标题和通知内容。

      3.用户编写完成后，点击确定按钮，后台程序根据点击的时间生成发布时间，将通知信息重新传送到后台服务器数据库中，并且网页跳转到“通知页面”页面。
...
+ 删除通知
    ...
    + 基本事件流：
      ...
      1.用户进入“通知页面”页面，选择列表中的想要删除的通知，可以单选也可以复选。点击下方的删除按钮。

      2.系统将页面中勾选的通知删除，并且通知后台服务器删除相关数据库。
...
+ 数据查询
    ...
    + 基本事件流
      ...
      1. 省端用户进入”数据查询“界面，在界面上可以对企业名称、企业编号、单位性质、所属行业、所在地区、起始月份和结束月份进行条件选择，然后进行查询。
      2. 在点击”查询“按钮后，跳转到“查询结果“界面，查询界面显示出符合条件的各个企业的企业信息概览，下拉可以查看具体数据。
      3. 在查询结果界面，点击“导出”按钮，可以将查询结果导出为Excel文件。
